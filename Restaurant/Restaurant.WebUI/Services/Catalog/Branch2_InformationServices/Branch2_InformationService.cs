...
namespace Restaurant.WebUI.Services.Catalog.Branch2_InformationServices
{
    public class Branch2_InformationService : IBranch2_InformationService
    {
        ...
        {
            await _httpClient.PostAsJsonAsync<CreateBranch2_Dto>("Branch2_Information", createBranc2_Dto);
        }

        public async Task DeleteBranch2_Information(string id)
        {
            await _httpClient.DeleteAsync($"Branch2_Information/{id}");
        }

        public async Task<List<ResultBranch2_Dto>> GetAllBranch2_Information()
        {
            var responseMessage =_httpClient.GetAsync("Branch2_Information");
        ...
        public async Task<GetByIdBranch2_Dto> GetByIdBranch2_InformationById(string id)
        {
            var responseMessage = _httpClient.GetAsync("Branch2_Information/"+id);
            if (responseMessage.Result.IsSuccessStatusCode)
            {
                var result = await responseMessage.Result.Content.ReadFromJsonAsync<GetByIdBranch2_Dto>();
                return result;
            }
            return null;
        }
        ...
        {
             await _httpClient.PutAsJsonAsync<UpdateBranch2_Dto>("Branch2_Information", updateBranc2_Dto);
        }
    }
}
